  ...
  $(function() {
    ...
    function initRegistrationChart() {
      ...
      if (typeof Chart === 'undefined') {
        ...
        setTimeout(initRegistrationChart, 100);
        return;
      }

      console.log('✅ Chart.js loaded');

      var canvas = document.getElementById('registration-chart');
      if (!canvas) {
        console.error('❌ Canvas element not found');
        return;
      }

      var monthsAttr = canvas.getAttribute('data-months');
      var countsAttr = canvas.getAttribute('data-counts');

      console.log('📊 Data from canvas attributes:');
      console.log('   data-months:', monthsAttr);
      console.log('   data-counts:', countsAttr);

      if (!monthsAttr || !countsAttr) {
      ...
      try {
        ...
        if (!months || !counts || months.length === 0 || counts.length === 0) {
        ...
        // Create chart with Chart.js v3+ syntax
    ...
    function initAppointmentStatusChart() {
      if (typeof Chart === 'undefined') {
        console.error('❌ Chart.js not loaded yet');
        setTimeout(initAppointmentStatusChart, 100);
        return;
      }

      var canvas = document.getElementById('appointment-status-chart');
      if (!canvas) {
        return;
      }

      var labelsAttr = canvas.getAttribute('data-labels');
      var valuesAttr = canvas.getAttribute('data-values');

      if (!labelsAttr || !valuesAttr) {
        console.error('❌ Missing data attributes for appointment chart');
        return;
      }
      ...
      try {
        ...
        if (!labels || !values || labels.length === 0 || values.length === 0) {
          console.error('❌ Data arrays are empty for appointment chart');
          return;
        }
        ...
        var colors = ['#28a745', '#ffc107', '#dc3545']; // Green, Amber, Red
        ...
        console.error('❌ Error initializing appointment chart:', e.message);
      }
    }

    initAppointmentStatusChart();

    // Initialize Patient Trimester Distribution Pie Chart
    function initTrimesterChart() {
      if (typeof Chart === 'undefined') {
        console.error('❌ Chart.js not loaded yet');
        setTimeout(initTrimesterChart, 100);
        return;
      }

      var canvas = document.getElementById('trimester-distribution-chart');
      if (!canvas) {
        return;
      }
      ...
      if (!labelsAttr || !valuesAttr) {
        console.error('❌ Missing data attributes for trimester chart');
        return;
      }
      ...
      try {
        ...
        if (!labels || !values || labels.length === 0 || values.length === 0) {
          console.error('❌ Data arrays are empty for trimester chart');
          return;
        }

        var ctx = canvas.getContext('2d');
        
        // Define colors for each trimester
        var colors = ['#007bff', '#20c997', '#e83e8c']; // Blue, Teal, Pink
        ...
        console.log('✅ Trimester Distribution Chart initialized');
      } catch(e) {
        console.error('❌ Error initializing trimester chart:', e.message);
      }
    }
    ...
    if ($("#sales-chart").length) {
      ...
      var labelsAttr = salesChartElement.getAttribute('data-labels');
      var valuesAttr = salesChartElement.getAttribute('data-values');

      var chartLabels = labelsAttr ? JSON.parse(labelsAttr) : ["Today Appointments", "Total Patients", "Total Doctors", "Total Payments"];
      var chartValues = valuesAttr ? JSON.parse(valuesAttr) : [0, 0, 0, 0];
      ...
      var SalesChart = new Chart(SalesChartCanvas, {
        ...
        data: {
          ...
          datasets: [{
            label: 'Overview',
            data: chartValues,
            backgroundColor: ['#4B49AC', '#98BDFF', '#7DA0FA', '#F3797E']
          }]
        },
        options: {
          cornerRadius: 5,
    ...
    if ($("#doctor-workload-chart").length) {
      var workloadChartElement = document.getElementById('doctor-workload-chart');
      var doctorNamesAttr = workloadChartElement.getAttribute('data-doctor-names');
      var patientCountsAttr = workloadChartElement.getAttribute('data-patient-counts');

      console.log('Doctor Workload Chart Data:');
      console.log('  Doctor Names:', doctorNamesAttr);
      console.log('  Patient Counts:', patientCountsAttr);

      var doctorNames = doctorNamesAttr ? JSON.parse(doctorNamesAttr) : [];
      var patientCounts = patientCountsAttr ? JSON.parse(patientCountsAttr) : [];

      console.log('Parsed Doctor Names:', doctorNames);
      console.log('Parsed Patient Counts:', patientCounts);

      if (doctorNames.length > 0 && patientCounts.length > 0) {
        var workloadCanvas = $("#doctor-workload-chart").get(0).getContext("2d");
        var workloadChart = new Chart(workloadCanvas, {
          type: 'bar',
          data: {
            labels: doctorNames,
            datasets: [{
        ...
        console.log('✅ Doctor Workload Chart initialized successfully');
      } else {
        console.log('⚠️ No doctor data available to render chart');
      }
    }
